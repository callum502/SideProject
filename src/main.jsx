...
function App() {
  ...
  return <div className="app">
    ...
    {modal?.attachMedia && <Modal title={`Add ${modal.attachMedia} from ${boulder.name} page`} close={() => { if (!busy) setModal(null); }}><AttachMediaForm boulder={boulder} problem={problem} kind={modal.attachMedia} busy={busy} error={error} cancel={() => setModal(null)} save={async (field, ids) => { if (await updateBoulder({ problems: boulder.problems.map(p => p.id === problem.id ? { ...p, [field]: [...new Set([...p[field], ...ids])] } : p) })) setModal(null); }}/></Modal>}
