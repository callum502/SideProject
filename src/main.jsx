...
function App() {
  ...
  return <div className="app">
    <aside className="sidebar"><a className="brand" href="#" onClick={e => { e.preventDefault(); goHome(); }}><span className="brand-mark">△</span> Side<span className="brand-light">Proj</span></a><div className="sidebar-title">YOUR FIELD GUIDE</div><a className="nav-active home-nav" href="#" onClick={goHome}><Icon name="pin"/> Find locations <span>{db.locations.length}</span></a><div className="sidebar-footer"><span className="status-dot"/> Local field guide<p>Built for the days outside.</p></div></aside>
    <div className="workspace"><header className="topbar"><span><a href="#" onClick={goHome}>Find locations</a>{place && <> / <strong>{place.name}</strong></>}{boulder && <> / {boulder.name}</>}</span><div className="header-actions"><button className="primary small" disabled={!loaded || busy} onClick={() => { setError(''); setModal('location'); }}>+ Create location</button>{place && <button className="secondary small" disabled={busy} onClick={download}>↓ Export guide</button>}<button className="secondary small" onClick={share}><Icon name="arrow"/> Copy local link</button></div></header>
    ...
    {home ? <><LocationSearch locations={db.locations} loaded={loaded}/><LocationMap locations={db.locations} selectedId={null} onSelect={id => choose(id)}/></> : <><a className="back-to-search" href="#" onClick={goHome}>← Back to search</a>
