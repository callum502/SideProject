...
function App() {
  ...
  useEffect(() => {
    const navigate = () => { const params = new URLSearchParams(location.hash.slice(1)); setLocationId(params.get('location')); setBoulderId(params.get('boulder')); setProblemId(params.get('problem')); setModal(null); window.scrollTo(0, 0); };
  ...
  return <div className="app">
    <div className="workspace"><header className="topbar"><a className="brand" href="#" aria-label="SideProj home"><span className="brand-mark" aria-hidden="true">△</span> Side<span className="brand-light">Proj</span></a><div className="header-actions"><button className="primary small" disabled={!loaded || busy} onClick={() => { setError(''); setModal('location'); }}>+ Create location</button></div></header>
    ...
    {home ? <><LocationSearch locations={db.locations} loaded={loaded}/><LocationMap locations={db.locations} selectedId={null} onSelect={id => choose(id)}/></> : <>
    ...
    {!boulder && <div className="detail-banner"><div className="eyebrow"><Icon name="pin"/> {place.region || 'CLIMBING LOCATION'}</div><h2>{place.name}</h2><div className="banner-bottom"><span>{countLabel(place.boulders.length, 'boulder')} · {countLabel(place.boulders.reduce((s, b) => s + (b.problems || []).length, 0), 'problem')} · {countLabel(place.boulders.reduce((s, b) => s + b.images.length, 0), 'photo')} · {countLabel(place.boulders.reduce((s, b) => s + (b.videos || []).length, 0), 'video')}</span><div className="location-actions"><button onClick={() => { setError(''); setModal('editLocation'); }}>Edit location ↗</button><button disabled={busy} onClick={() => { setError(''); setModal({ removeLocation: place }); }}>Remove location</button></div></div><PageNavigation place={place} boulder={boulder} problem={problem} problemId={problemId}/></div>}
    {boulder && !problemId && <div className="detail-banner boulder-banner"><div className="eyebrow">BOULDER</div><h1>{boulder.name}</h1><div className="banner-bottom"><span>{countLabel((boulder.problems || []).length, 'problem')} · {countLabel(boulder.images.length, 'photo')} · {countLabel((boulder.videos || []).length, 'video')}</span><div className="location-actions"><button disabled={busy} onClick={() => { setError(''); setModal('editBoulder'); }}>Edit boulder</button></div></div><PageNavigation place={place} boulder={boulder} problem={problem} problemId={problemId}/></div>}
    {problemId && boulder ? <ProblemPage navigation={<PageNavigation place={place} boulder={boulder} problem={problem} problemId={problemId}/>} problem={problem} boulder={boulder} busy={busy} remove={() => { setError(''); setModal({ removeProblem: problem }); }} edit={() => { setError(''); setModal({ problemForm: true, problem }); }} Shapes={Shapes}/> : boulder ? <><div className="notes-block"><h3>Finding the Boulder</h3><p className="preserve">{boulder.notes || 'Describe how to find this boulder after following the location approach notes. Include nearby landmarks, paths, and distances.'}</p></div><ProblemList place={place} boulder={boulder} busy={busy} add={() => { setError(''); setModal({ problemForm: true }); }}/><div className="section-heading photos-heading"><div><h3>Photos & Topos</h3><p>An easier way to share different lines.</p></div><label className={`secondary small upload ${busy ? 'disabled' : ''}`}>+ Upload images<input type="file" accept="image/jpeg,image/png,image/webp" multiple disabled={busy} onChange={upload}/></label></div>{!boulder.images.length ? <div className="photo-empty"><Icon name="photo"/><h3>Upload an image here.</h3><p>Upload a boulder photo, then annotate to highlight different problems.</p><span>JPEG, PNG, WebP · up to 8 MB each</span></div> : <div className="photo-grid">{boulder.images.map(img => <div className="photo-item" key={img.id}><button className="photo-card" onClick={() => setModal({ image: img })}><div className="photo-thumb"><img src={img.url} alt={img.name}/><Shapes items={img.annotations}/></div><div><span>{img.name}</span><strong>Annotate ↗</strong></div></button><button className="remove-media" disabled={busy} aria-label={`Remove image ${img.name}`} onClick={() => { setError(''); setModal({ remove: img, collection: 'images' }); }}>Remove image</button></div>)}</div>}
    <section aria-labelledby="beta-videos-heading"><div className="section-heading photos-heading"><div><h3 id="beta-videos-heading">Beta videos</h3><p>Steal beta to become a better climber.</p></div><label className={`secondary small upload ${busy ? 'disabled' : ''}`}>{busy ? 'Please wait...' : '+ Upload video'}<input type="file" accept="video/mp4,video/webm,video/quicktime,.mov" multiple disabled={busy} onChange={uploadVideos}/></label></div>
    ...
    {modal?.problemForm && <Modal title={modal.problem ? 'Edit problem' : 'Add problem'} close={() => { if (!busy) setModal(null); }}><ProblemForm boulder={boulder} problem={modal.problem} busy={busy} error={error} cancel={() => setModal(null)} save={async item => { const problems = modal.problem ? boulder.problems.map(p => p.id === item.id ? item : p) : [...(boulder.problems || []), item]; if (await updateBoulder({ problems })) { setModal(null); choose(place.id, boulder.id, item.id); } }}/></Modal>}
