...
function App() {
  const [user, setUser] = useState(null), [loginPage, setLoginPage] = useState(location.hash === '#login');
  ...
  useEffect(() => {
    const navigate = () => { setLoginPage(location.hash === '#login'); const params = new URLSearchParams(location.hash.slice(1)); setLocationId(params.get('location')); setBoulderId(params.get('boulder')); setProblemId(params.get('problem')); setModal(null); window.scrollTo(0, 0); };
  ...
  return <div className="app">
    <div className="workspace"><header className="topbar"><a className="brand" href="#" aria-label="SideProj home"><span className="brand-mark" aria-hidden="true">△</span> Side<span className="brand-light">Proj</span></a><div className="header-actions">{home && !loginPage && user && <button className="primary small" disabled={!loaded || busy} onClick={() => { setError(''); setModal('location'); }}>+ Create location</button>}{user ? <><span className="signed-in">{user.name} <small>{user.role}</small></span><button className="secondary small" onClick={logout}>Log out</button></> : <a className="secondary small" href="#login">Log in</a>}</div></header>
    {loginPage ? <LoginPage onLogin={identity => { setUser(identity); location.hash = ''; }}/>: <main className={home ? "search-home" : "location-page"}>
