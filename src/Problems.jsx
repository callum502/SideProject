...
export function ProblemList({ place, boulder, add, busy }) {
  return <section className="problems-section"><div className="section-heading photos-heading"><h3>Problems <span className="count">{(boulder.problems || []).length}</span></h3><button className="secondary small" disabled={busy} onClick={add}>+ Add problem</button></div>
    {boulder.problems?.length ? <div className="problem-list">{boulder.problems.map(p => <a className="problem-row" key={p.id} href={problemHref(place.id, boulder.id, p.id)}><span><strong>{p.name}</strong><p>{p.description}</p></span><span className="grade">{p.grade}</span><span aria-hidden="true">↗</span></a>)}</div> : <p className="problem-empty">No problems yet. Add the first line on this boulder.</p>}
  </section>;
}
...
export function ProblemPage({ problem, boulder, edit, Shapes }) {
  ...
  return <article className="problem-page"><div className="boulder-heading"><div><div className="eyebrow">PROBLEM</div><h1>{problem.name}</h1><span className="grade">{problem.grade}</span></div><button className="secondary small" onClick={edit}>Edit problem</button></div>
