...
export function ProblemPage({ problem, boulder, edit, remove, busy, Shapes, navigation, canEdit }) {
  ...
  return <article className="problem-page"><div className="detail-banner problem-banner">{navigation}<div className="eyebrow">PROBLEM</div><h1>{problem.grade} - {problem.name}</h1><p className="created-by">Created by {problem.createdByName || problem.createdBy || 'Admin'}</p><div className="banner-bottom"><span>{images.length} {images.length === 1 ? 'photo' : 'photos'} · {videos.length} {videos.length === 1 ? 'video' : 'videos'}</span><div className="location-actions"><button disabled={busy || !canEdit} onClick={edit}>Edit problem</button><button className="banner-remove" disabled={busy || !canEdit} onClick={remove}>Remove problem</button></div></div></div>
