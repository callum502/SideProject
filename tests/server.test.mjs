...
test('guide, uploaded images and all annotation types persist; stale writes and invalid uploads are rejected', async () => {
  ...
  const start = async () => { app = await createGuideServer({ dataDir }); await new Promise(r => app.server.listen(0, '127.0.0.1', r)); const base = 'http://127.0.0.1:' + app.server.address().port; const login = await globalThis.fetch(base + '/api/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: 'Admin', password: 'Password' }) }); cookie = login.headers.get('set-cookie').split(';')[0]; return base; };
