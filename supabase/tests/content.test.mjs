...
test('PostgreSQL content API round-trips nested data, enforces RLS and rolls back failed batches', async () => {
  ...
  try {
    ...
    for (const name of ['20260909000100_initial_schema.sql','20260909000200_account_access.sql','20260910000100_content_api.sql']) await db.exec(await readFile(new URL('../migrations/'+name,import.meta.url),'utf8'));
    ...
    const store=createContentStore({env:{SUPABASE_URL:'https://fixture.example',SUPABASE_PUBLISHABLE_KEY:'test'},fetchImpl:async(url,options)=>{
    ...
    guide=await store.save(guide,userA,a,dataDir);
    ...
    guide=await store.save(guide,userB,b,dataDir);
    assert.equal(guide.locations[0].boulders[0].createdBy,b);
    await assert.rejects(store.save(stale,userA,a,dataDir),error=>error.status===409);
    const otherEdit=structuredClone(guide);otherEdit.locations[0].name='Not yours';
    await assert.rejects(store.save(otherEdit,userB,b,dataDir),error=>error.status===403);
    ...
    ({server}=await createGuideServer({dataDir,contentStore:store,auth}));
    await writeFile(path.join(dataDir,'uploads',filename),Buffer.from([137,80,78,71,13,10,26,10]));
    rock.images.push({id:imageId,name:'Photo',url:'/uploads/'+filename,annotations:[{type:'line',x:0,y:0,x2:100,y2:100}]});
    ...
    await assert.rejects(store.save(guide,userB,b,dataDir),error=>error.status===409);
    ...
    await assert.rejects(store.save(remove,userA,a,dataDir),error=>error.status===403);
    assert.deepEqual((await store.save(remove,userAdmin,admin,dataDir)).locations,[]);
